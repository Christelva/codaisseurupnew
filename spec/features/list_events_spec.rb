...
describe "Current user viewing the list of events" do
  ...
  let!(:event1) { create :room, event_name: "Listing Name 1", user: user }
  let!(:event2) { create :room, event_name: "Listing Name 2", user: user }
  let!(:event3) { create :room, event_name: "Another users event", user: another_user }
